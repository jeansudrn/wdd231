...
function renderizarCursos(listaFiltrada) {
    ...
    listaFiltrada.forEach(curso => {
        ...
        // Se estiver concluído, adiciona a classe CSS específica
        if (curso.completed) {
            card.classList.add("concluido");
        }
        
        card.innerHTML = `<strong>${curso.subject} ${curso.number}</strong>`;
        container.appendChild(card);
    });

    // Calcula e atualiza a soma dos créditos dinamicamente com reduce
...
// Inicializa a página mostrando todos os cursos
